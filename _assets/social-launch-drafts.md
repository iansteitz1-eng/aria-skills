...
The pre-launch ceremony for downloads.ariacode.io tonight: ~2 minutes.
```
...
Solo: clone, run, free forever.
Hosted: staycool.ai/aria-code, BYOK, free tier.
```
...
I open-sourced these because the skills are the commodity. The hosted version (https://staycool.ai/aria-code) is the actual product — team approval, scheduled runs, audit, multi-user. Free tier exists; no credit card.
...
(Disclosure: I run staycool.ai/aria-code — these skills are open source separately; the hosted platform is where they get team approval / audit / scheduling, but solo use is fine forever.)
```
...
Each post links back to the GitHub repo. The GitHub repo's README links to `staycool.ai/aria-code`. The funnel runs itself.
...
2. Verify `downloads.ariacode.io/skills/` resolves (already confirmed 200)
3. Verify `staycool.ai/aria-code` is the right destination URL for the CTAs (it is)
