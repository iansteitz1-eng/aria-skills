...
1. **Pick the sprint number.** Look at `/opt/aria/v4/sprints/` — find the highest existing NNN and add 1. Slug is lowercase-hyphenated.

   ```sh
   ls /opt/aria/v4/sprints/ | sort -n | tail -3
   ```
...
2. **Create the folder + subdirs:**
   ...
   ```sh
   SPRINT=021_<slug>
   mkdir -p /opt/aria/v4/sprints/$SPRINT/{reference,sql,.claude/agents}
   ```
